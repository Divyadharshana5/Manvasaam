...
export default function RetailDashboard() {
  ...
  const recentActivities = [
    {
      type: "order",
      message: "New order from Green Valley Farm",
      time: "2 hours ago",
      icon: ShoppingCart,
    },
    {
      type: "delivery",
      message: "Delivery completed for ORD-001",
      time: "4 hours ago",
      icon: Truck,
    },
    {
      type: "stock",
      message: "Low stock alert: Organic Milk",
      time: "6 hours ago",
      icon: AlertCircle,
    },
    {
      type: "payment",
      message: "Payment received ₹8,500",
      time: "8 hours ago",
      icon: IndianRupee,
    },
  ];
  ...
  const quickActions = [
    {
      name: "Place Order",
      icon: Plus,
      href: "/dashboard/retail/orders/new",
      color: "bg-green-500",
    },
    {
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 animate-fade-in-up">
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <Input
              placeholder={
                t?.orders?.searchPlaceholder ?? "Search orders, products..."
              }
              value={searchQuery}
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">
                  {t?.dashboard?.todayOrders ?? "Today's Orders"}
                </p>
        ...
        <Card className="border-l-4 border-l-purple-500 stat-card card-glow animate-fade-in-up stagger-3">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">
                  {t?.dashboard?.newCustomers ?? "New Customers"}
                </p>
        ...
        <TabsContent value="overview" className="space-y-4 tab-content">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <Card className="animate-scale-in stagger-1">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">
                  {t?.orders?.cardTitle ?? "Total Orders"}
                </CardTitle>
            ...
            <Card className="animate-scale-in stagger-2">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">
                  {t?.dashboard?.totalRevenue ?? "Monthly Revenue"}
                </CardTitle>
            ...
            <CardHeader>
              ...
              <CardDescription>
                {t?.dashboard?.quickActionsDesc ??
                  "Frequently used actions for your retail operations"}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <TabsContent value="inventory" className="space-y-4 tab-content">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold">
              {t?.products?.title ?? "Inventory Management"}
            </h3>
            <div className="flex gap-2">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  <DropdownMenuLabel>
                    {t?.filters?.byCategory ?? "Filter by Category"}
                  </DropdownMenuLabel>
                  <DropdownMenuCheckboxItem
                  ...
                  <DropdownMenuLabel>
                    {t?.filters?.stockStatus ?? "Stock Status"}
                  </DropdownMenuLabel>
                  <DropdownMenuCheckboxItem
          ...
          <div className="grid gap-4">
            {filteredInventory.map((item, index) => (
              <Card
                key={index}
                className={`animate-slide-in-left list-item ${
                      ...
                      <div className="w-24">
                        ...
                        {item.stock <= item.minStock && (
                          <p className="text-xs text-orange-600 mt-1">
                            {t?.products?.lowStockIndicator ?? "Low Stock!"}
                          </p>
                        )}
                      </div>
                      <Button
                    ...
                    <div className="text-right">
                      ...
                      <div className="flex gap-2 mt-2">
                        ...
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            handleTrackOrder(order.id, order.status)
                          }
                          title="Track order"
                        >
