...
export default function SuppliersPage() {
  ...
  const handleOrderFromSupplier = (supplier: any) => {
    ...
    const orderUrl = `/dashboard/retail/orders/new?supplier=${
      supplier.id
    }&supplierName=${encodeURIComponent(supplier.name)}`;
    alert(
      `Redirecting to place order from ${supplier.name}...\n\nThis would navigate to: ${orderUrl}`
    );
    ...
    <div className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8 page-transition">
      ...
      <div className="flex items-center gap-4 animate-fade-in-up">
        ...
        <div className="flex-1">
          <h1 className="text-3xl font-bold tracking-tight">
            {t?.retail?.suppliersTitle ?? "Supplier Management"}
          </h1>
          <p className="text-muted-foreground">
            {t?.retail?.suppliersDesc ??
              "Manage your supplier relationships and communications"}
          </p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {filteredSuppliers.map((supplier, index) => (
          <Card
            key={supplier.id}
            className={`hover:shadow-lg transition-shadow animate-fade-in-up list-item stagger-${
              (index % 4) + 1
            }`}
          >
            <CardHeader>
      ...
      <Dialog open={messageDialog} onOpenChange={setMessageDialog}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Send a message to your supplier about orders, delivery, or general
              inquiries.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                id="subject"
                placeholder="e.g., Inquiry about fresh vegetables"
              />
            </div>
            <div className="space-y-2">
      ...
      <Dialog open={addSupplierDialog} onOpenChange={setAddSupplierDialog}>
        <DialogContent className="sm:max-w-[425px] max-h-[85vh] overflow-y-auto bg-white text-gray-900">
          ...
          <div className="flex gap-2">
            <Button
              onClick={() => setAddSupplierDialog(false)}
              variant="outline"
              className="flex-1"
            >
              Cancel
