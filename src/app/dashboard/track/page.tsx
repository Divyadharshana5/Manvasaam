...
const mockTrackingData = {
  ORD002: {
    path: [
      ...
      {
        name: "State Warehouse, Maharashtra",
  ...
  ORD005: {
    path: [
      ...
      {
        name: "State Warehouse, Tamil Nadu",
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4 md:p-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-3xl font-bold tracking-tight">
            {t?.track?.title ?? t?.sidebar?.track ?? "Live Order Tracking"}
          </h2>
          <p className="text-muted-foreground">
            {t?.track?.watchingOrder ??
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {t?.track?.deliveryTitle ?? "Your Delivery is on its way!"}
          </CardTitle>
          <CardDescription>
            {t?.track?.eta ?? "Estimated Time of Arrival:"}
        ...
        <CardContent>
          <div className="relative w-full h-96 bg-muted rounded-lg overflow-hidden border">
            ...
            <div
              ...
              style={{
                backgroundImage:
                  "url('https://placehold.co/1000x800.png?text=Map+View')",
              }}
            ...
            {trackingInfo.path.map((step, index) => (
              ...
              >
                ...
                <span className="text-xs text-muted-foreground -translate-x-1/2 relative left-1/2 text-center block mt-1 whitespace-nowrap">
                  {step.name}
                </span>
              </div>
            ))}
          ...
          <div className="mt-6 space-y-2">
            <h3 className="font-semibold">
              ...
              <span className="text-primary">{currentStatus}</span>
            </h3>
        ...
        <CardFooter className="flex justify-end">
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{`${
                  t?.orders?.cancelDialogTitle ?? "Cancel Order"
                } ${orderId}`}</DialogTitle>
                <DialogDescription>
              ...
              <Form {...form}>
                <form
                  onSubmit={form.handleSubmit(onCancelSubmit)}
                  className="space-y-4"
                >
                  <FormField
                        ...
                        <FormControl>
                          ...
                          >
                            <FormItem className="flex items-center space-x-3 space-y-0">
                              <FormControl>
                                <RadioGroupItem value="mistake" />
                              </FormControl>
                              <FormLabel className="font-normal">
                                {t?.orders?.cancelReasonMistake ??
                                  "Ordered by mistake"}
                              </FormLabel>
                            </FormItem>
                            <FormItem className="flex items-center space-x-3 space-y-0">
                              <FormControl>
                                <RadioGroupItem value="timing" />
                              </FormControl>
                              <FormLabel className="font-normal">
                                {t?.orders?.cancelReasonTiming ??
                                  "Delivery timing is not suitable"}
                              </FormLabel>
                            </FormItem>
                            <FormItem className="flex items-center space-x-3 space-y-0">
                              <FormControl>
                            ...
                            <FormItem className="flex items-center space-x-3 space-y-0">
                              ...
                              <FormLabel className="font-normal">
                                {t?.orders?.cancelReasonOther ?? "Other"}
                              </FormLabel>
                            </FormItem>
                          </RadioGroup>
                        </FormControl>
                  ...
                  {selectedReason === "other" && (
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            {t?.orders?.cancelPleaseSpecify ?? "Please specify"}
                          </FormLabel>
                          <FormControl>
                            <Textarea
