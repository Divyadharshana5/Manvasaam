...
export default function MaintenancePage() {
  ...
  const handleScheduleService = (vehicleId: string) => {
    const vehicle = vehicles.find((v) => v.id === vehicleId);
    if (vehicle) {
      const serviceDate = prompt(
        `Schedule Service for ${vehicleId}\n\nEnter service date (YYYY-MM-DD):`
      );
      if (serviceDate) {
        alert(
...
${
  vehicle.issues.length > 0
    ? vehicle.issues.map((issue) => `• ${issue}`).join("\n")
    : "• No issues reported"
}
...
• Health Score Trend: ${
  ...
  const handleNewScheduleService = () => {
    const availableVehicles = vehicles
      .map((v) => `${v.id} (${v.model})`)
      .join("\n");
    const selectedVehicle = prompt(
      `Schedule New Service\n\nAvailable Vehicles:\n${availableVehicles}\n\nEnter Vehicle ID:`
    );

    if (selectedVehicle) {
      const vehicle = vehicles.find(
        (v) => v.id === selectedVehicle.toUpperCase()
      );
      if (vehicle) {
        const serviceDate = prompt(
          `Schedule Service for ${vehicle.id}\n\nEnter service date (YYYY-MM-DD):`
        );
        if (serviceDate) {
          ...
          const selectedType =
            serviceTypes[parseInt(serviceType || "1") - 1] || "Regular Service";
          ...
          console.log(
            `New service scheduled: ${vehicle.id} - ${selectedType} on ${serviceDate}`
          );
        }
      } else {
  ...
  const handleExportReport = () => {
    ...
    csvContent +=
      "Vehicle ID,Model,Status,Health Score,Mileage (km),Fuel Efficiency (km/L),Last Service,Next Service,Service Type,Maintenance Cost (₹),Downtime (days),Issues,Upcoming Maintenance\n";
    vehicles.forEach((vehicle) => {
      csvContent += `${vehicle.id},${vehicle.model},${vehicle.status},${
        vehicle.healthScore
      }%,${vehicle.mileage},${vehicle.fuelEfficiency},${vehicle.lastService},${
        vehicle.nextService
      },${vehicle.serviceType},${vehicle.maintenanceCost},${
        vehicle.downtime
      },"${vehicle.issues.join("; ")}","${vehicle.upcomingMaintenance.join(
        "; "
      )}"\n`;
    });
    ...
    csvContent +=
      "Vehicle ID,Service Type,Date,Estimated Cost (₹),Duration,Priority,Description\n";
    ...
    csvContent +=
      "Vehicle ID,Service Type,Date,Cost (₹),Duration,Status,Description\n";
    ...
    alert(
      `Maintenance Report Exported!\n\nFile: Fleet_Maintenance_Report_${
          ...
          <Card>
            <CardContent className="p-4">
              <div className="relative flex items-center">
                ...
                <Input
                  placeholder={
                    t?.transport?.searchVehiclesPlaceholder ??
                    "Search vehicles by ID or model..."
                  }
                  value={searchQuery}
                ...
                <CardContent className="p-6">
                  ...
                  <div className="grid gap-4 md:grid-cols-2 mb-4">
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        {vehicle.upcomingMaintenance.map(
                          (maintenance, index) => (
                            <Badge
                              key={index}
                              variant="outline"
                  ...
                  <div className="flex gap-2">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleServiceDetails(vehicle.id)}
                    >
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleHealthReport(vehicle.id)}
                    >
        ...
        <TabsContent value="schedule" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Planned maintenance activities for your fleet
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="font-semibold">
                          {schedule.vehicleId} - {schedule.type}
                        </h4>
                        <p className="text-sm text-muted-foreground">
        ...
        <TabsContent value="history" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Recent maintenance activities and costs
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {maintenanceHistory.map((history, index) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="font-semibold">
                          {history.vehicleId} - {history.type}
                        </h4>
                        <p className="text-sm text-muted-foreground">
                          {history.description}
                        </p>
