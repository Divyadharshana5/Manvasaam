...
export default function VoiceAssistantHelpPage() {
  ...
  const voiceCommands = [
    ...
    { command: "Go to FAQ", description: "Access help and FAQ" },
    {
      ...
      description: "Navigate to hub dashboard (for hub users)",
    },
    {
      command: "Show inventory",
      description: "View inventory (for hub users)",
    },
    { command: "Go to farmers", description: "Manage farmers (for hub users)" },
    {
      command: "Show deliveries",
      description: "View deliveries (for hub users)",
    },
    {
      command: "Open analytics",
      description: "View analytics (for hub users)",
    },
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 p-4">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="text-center space-y-4">
          <div className="flex items-center justify-center gap-2">
            ...
            <h1 className="text-3xl font-bold">
              {t?.voiceAssistant?.helpTitle || "Voice Assistant Help"}
            </h1>
          </div>
          <p className="text-lg text-muted-foreground">
            {t?.voiceAssistant?.helpDescription ||
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {t?.voiceAssistant?.howToUseDescription ||
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {t?.voiceAssistant?.tipsDescription ||
            ...
            <CardContent className="space-y-3">
              <div className="space-y-2">
                <p className="text-sm">
                  <strong>Speak clearly:</strong> Use a normal speaking pace and
                  volume
                </p>
                <p className="text-sm">
                  <strong>Use keywords:</strong> You can say "go to", "open",
                  "show", or "navigate to"
                </p>
                <p className="text-sm">
                  <strong>Browser permissions:</strong> Allow microphone access
                  when prompted
                </p>
                <p className="text-sm">
                  <strong>Quiet environment:</strong> Works best in low-noise
                  environments
                </p>
                <p className="text-sm">
                  <strong>Dedicated assistant:</strong> Only responds to
                  navigation commands
                </p>
              </div>
            </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {t?.voiceAssistant?.availableCommandsTitle ||
                "Available Voice Commands"}
            </CardTitle>
            <CardDescription>
              {t?.voiceAssistant?.availableCommandsDescription ||
        ...
        <div className="text-center">
          <Link href="/dashboard">
            <Button>
              {t?.voiceAssistant?.backToDashboard || "Back to Dashboard"}
            </Button>
          </Link>
        </div>
      </div>
