...
function AutoFaceDetectionDisplay({ 
  isDetecting, 
  detectedUser, 
  countdown 
}: { 
  ...
  if (!isDetecting && !detectedUser) return null;
      ...
      <div className="bg-white rounded-lg p-6 max-w-sm mx-4 text-center">
        {isDetecting ? (
          <div className="space-y-4">
            ...
            <h3 className="font-semibold text-lg">Detecting Face...</h3>
          ...
          <div className="space-y-4">
            ...
            <h3 className="font-semibold text-lg text-green-600">Face Recognized!</h3>
            <div className="space-y-2">
              <p className="font-medium">Welcome back, {detectedUser.username}!</p>
              <p className="text-sm text-muted-foreground">
            ...
            <div className="space-y-2">
              ...
              <Progress value={(3 - countdown) * 33.33} className="h-2" />
            </div>
          </div>
        ) : null}
      </div>
    ...
    <div className="space-y-3 p-4 bg-muted/50 rounded-lg border">
      ...
      <div className="grid grid-cols-2 gap-2 text-sm">
        <div className="flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full ${analysis.positioning.centered ? 'bg-green-500' : 'bg-red-500'}`} />
          <span>Centered</span>
        </div>
        <div className="flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full ${analysis.positioning.frontFacing ? 'bg-green-500' : 'bg-red-500'}`} />
          <span>Front Facing</span>
        </div>
        <div className="flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full ${analysis.positioning.eyesVisible ? 'bg-green-500' : 'bg-red-500'}`} />
          <span>Eyes Visible</span>
        </div>
        <div className="flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full ${analysis.positioning.mouthVisible ? 'bg-green-500' : 'bg-red-500'}`} />
      ...
      {analysis.feedback && (
        <Alert className={analysis.suitableForAuth ? "border-green-200" : "border-yellow-200"}>
                        ...
                        render={({ field }) => (
                            <FormItem>
                                <FormLabel className="flex items-center gap-2">
                                    ...
                                    Face Registration (Auto-Login Enabled)
                                </FormLabel>
                                            ...
                                            {!facePhoto ? (
                                                <div className="space-y-2">
                                                    <Alert className="border-blue-200">
                                                        ...
                                                        <AlertDescription className="text-sm">
                                                            Register your face once for automatic login next time!
                                                        </AlertDescription>
                                                    </Alert>
                                                    ...
                                                    <Button 
                                                        ...
                                                        disabled={!isCameraActive || hasCameraPermission === false || analyzingFace} 
                                                        className="w-full"
                                                    >
                                                        {analyzingFace ? (
                                                            <>
                                                ...
                                                <div className="space-y-4">
                                                    <Alert className="border-green-200">
                                                        <CheckCircle className="h-4 w-4 text-green-600" />
                                                        <AlertDescription className="text-sm">
                                                            Face registered! You can now login automatically next time.
                                                        </AlertDescription>
                                                    </Alert>
...
export default function AutoFaceLoginPage() {
  const [loading, setLoading] = useState(false);
  const { toast } = useToast();
  const router = useRouter();
  const [activeTab, setActiveTab] = useState("login");
  const [authMode, setAuthMode] = useState("face"); // Default to face login
  const videoRef = useRef<HTMLVideoElement>(null);
  const [hasCameraPermission, setHasCameraPermission] = useState<boolean | null>(null);
  const [isCameraActive, setIsCameraActive] = useState(false);
  const { t } = useLanguage();
  const [showPassword, setShowPassword] = useState(false);
  const [faceLoginAnalysis, setFaceLoginAnalysis] = useState<FaceAnalysis | null>(null);
  ...
  // Auto face detection function
  const performAutoFaceDetection = useCallback(async () => {
    if (!videoRef.current || !isCameraActive || loading || detectedUser) return;
    ...
    try {
      ...
      if (analysis.faceDetected && analysis.suitableForAuth && analysis.confidence > 0.8) {
        // Attempt face login
        ...
        if (response.ok) {
          ...
          // Start countdown
          let countdown = 3;
          setAutoLoginCountdown(countdown);
          ...
          countdownIntervalRef.current = setInterval(() => {
            ...
            if (countdown <= 0) {
              ...
              signInWithCustomToken(auth, token).then(() => {
                toast({ 
                  title: "Auto Face Login Successful", 
      ...
      console.log('Auto-detection attempt failed:', error);
    }
  }, [isCameraActive, loading, detectedUser, toast, router]);

  // Start auto-detection when face login tab is active
  useEffect(() => {
    if (activeTab === 'login' && authMode === 'face' && isCameraActive && !detectedUser) {
      setIsAutoDetecting(true);
      
      // Start auto-detection every 2 seconds
      autoDetectionIntervalRef.current = setInterval(() => {
        performAutoFaceDetection();
      }, 2000);
  ...
  }, [activeTab, authMode, isCameraActive, detectedUser, performAutoFaceDetection]);
  ...
  async function onRegister(values: z.infer<typeof registerSchema>) {
    ...
    try {
      ...
      toast({
        ...
        description: "Your account has been created with auto-login enabled. Please log in.",
      });
      setActiveTab("login");
  ...
  return (
    <Card className="w-full max-w-md bg-card/80 backdrop-blur-lg border-2 border-primary/20 shadow-lg">
      <CardHeader className="text-center">
        ...
        <CardDescription>AI-powered face recognition with automatic login</CardDescription>
      </CardHeader>
      <CardContent>
            ...
            <Tabs value={authMode} onValueChange={setAuthMode} className="w-full">
              ...
              <TabsContent value="face" className="pt-4 space-y-4">
                <Alert className="border-blue-200">
                  ...
                  <AlertTitle>Smart Auto-Login Active</AlertTitle>
                  <AlertDescription className="text-sm">
                    Just look at the camera! If you're registered, you'll be logged in automatically.
                  </AlertDescription>
                </Alert>
                ...
                <div className="relative aspect-video w-full overflow-hidden rounded-lg border bg-muted">
                    ...
                    <AutoFaceDetectionDisplay 
                      isDetecting={isAutoDetecting && !detectedUser}
                      detectedUser={detectedUser}
                      countdown={autoLoginCountdown}
                    />
                </div>
                
                {faceLoginAnalysis && !detectedUser && (
                    <FaceAnalysisDisplay analysis={faceLoginAnalysis} />
                )}

                {!detectedUser && (
                    <Button 
                        onClick={handleManualFaceLogin} 
                        className="w-full" 
